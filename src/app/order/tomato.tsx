...
const tomato: TomatoItem[] = [
  { 
    id: 1, 
    name: 'Fried King Prawn with Tomato', 
  ...
  { 
    id: 2, 
    name: 'Fried Chicken with Tomato', 
    description: 'Including boiled rice OR chips OR fried rice (50p extra)', 
    price: 7.30,
...
const Tomato: React.FC<TomatoProps> = ({ addToCart }) => {
  ...
  const handleAddToCart = (item: TomatoItem) => {
  ...
  const availableCount = tomato.filter(tomato => !tomato.unavailable).length;
  const totalCount = tomato.length;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold">Tomato Dishes</h2>
      ...
      <div className="grid grid-cols-1 gap-4">
        {tomato.map((tomato) => (
          <div key={tomato.id} className="bg-white p-4 rounded-lg shadow-md flex justify-between items-center">
            <div>
              <h3 className={`text-lg font-bold ${tomato.unavailable ? 'text-gray-400' : 'text-gray-900'}`}>
                {tomato.name}
                {tomato.unavailable && <span className="ml-2 text-sm text-red-500">(Unavailable)</span>}
              </h3>
              <p className={`text-sm ${tomato.unavailable ? 'text-gray-400' : 'text-gray-600'}`}>
                {tomato.description}
              </p>
              <p className={`text-sm ${tomato.unavailable ? 'text-gray-400' : 'text-gray-600'}`}>
                {tomato.description}
              ...
              {!tomato.unavailable && (
                <div className="mt-2 space-y-2">
                    ...
                    {sideOptions.map((option) => (
                      <label key={option.id} className="flex items-center text-sm">
                        <input
                          ...
                          name={`side-${tomato.id}`}
                          value={option.id}
                          checked={(selectedSides[tomato.id] || 'rice') === option.id}
                          onChange={() => handleSideChange(tomato.id, option.id)}
                  ...
                  <p className="text-xs text-gray-500">{tomato.includes}</p>
                </div>
              )}
              {!tomato.unavailable && (
                <p className="text-red-600 font-medium mt-1">
                  £{tomato.price.toFixed(2)}
                  {selectedSides[tomato.id] === 'fried-rice' && (
                    <span className="text-gray-500 ml-1">+ £0.50 = £{(tomato.price + 0.5).toFixed(2)}</span>
                  )}
                </p>
              )}
            </div>
            <button 
              onClick={() => !tomato.unavailable && handleAddToCart(tomato)}
              disabled={tomato.unavailable}
