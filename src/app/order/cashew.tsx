...
const Cashew: React.FC<CashewProps> = ({ addToCart }) => {
  ...
  const handleAddToCart = (item: CashewItem) => {
  ...
  const availableCount = cashew.filter(cashew => !cashew.unavailable).length;
  const totalCount = cashew.length;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold">Cashew Dishes</h2>
      ...
      <div className="grid grid-cols-1 gap-4">
        {cashew.map((cashew) => (
          <div key={cashew.id} className="bg-white p-4 rounded-lg shadow-md flex justify-between items-center">
            <div>
              <h3 className={`text-lg font-bold ${cashew.unavailable ? 'text-gray-400' : 'text-gray-900'}`}>
                {cashew.name}
                {cashew.unavailable && <span className="ml-2 text-sm text-red-500">(Unavailable)</span>}
              </h3>
              <p className={`text-sm ${cashew.unavailable ? 'text-gray-400' : 'text-gray-600'}`}>
                {cashew.description}
              </p>
              {cashew.additionalInfo && (
                <p className={`text-sm ${cashew.unavailable ? 'text-gray-400' : 'text-amber-600'}`}>
                  {cashew.additionalInfo}
                </p>
              )}
              {!cashew.unavailable && (
                    ...
                    {sideOptions.map((option) => (
                      <label key={option.id} className="flex items-center text-sm">
                        <input
                          ...
                          name={`side-${cashew.id}`}
                          value={option.id}
                          checked={(selectedSides[cashew.id] || 'rice') === option.id}
                          onChange={() => handleSideChange(cashew.id, option.id)}
              ...
              {!cashew.unavailable && (
                <p className="text-red-600 font-medium mt-1">
                  £{cashew.price.toFixed(2)}
                  {selectedSides[cashew.id] === 'fried-rice' && (
                    <span className="text-gray-500 ml-1">+ £0.50 = £{(cashew.price + 0.5).toFixed(2)}</span>
                  )}
                </p>
              )}
            </div>
            <button 
              onClick={() => !cashew.unavailable && handleAddToCart(cashew)}
              disabled={cashew.unavailable}
